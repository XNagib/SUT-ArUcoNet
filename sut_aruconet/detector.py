"""Public SUT-ArUcoNet v1 inference pipeline."""
...
from .decoder import decode_rectified
...
SUPPORTED_DICTIONARIES = {"4x4_1000", "mip36h12"}
...
class SUTArUcoNet:
    def __init__(self, model_dir:str|Path|None=None, device:str='auto')->None:
        ...
        self.manifest=json.loads((self.model_dir/'model_manifest.json').read_text(encoding='utf-8-sig')); self.defaults=dict(self.manifest.get('runtime_defaults',{}))
    ...
    def detect_image(self,image:np.ndarray,imgsz:int|None=None,conf:float|None=None,iou:float|None=None,max_hamming:int|None=None,padding:float|None=None,dictionary:str|None=None,marker_size:float=0.0,camera_matrix:np.ndarray|None=None,distortion:np.ndarray|None=None)->dict[str,Any]:
        imgsz=int(imgsz if imgsz is not None else self.defaults.get('imgsz',416)); conf=float(conf if conf is not None else self.defaults.get('conf',0.25)); iou=float(iou if iou is not None else self.defaults.get('iou',0.5)); max_hamming=int(max_hamming if max_hamming is not None else self.defaults.get('max_hamming',0)); padding=float(padding if padding is not None else self.defaults.get('padding',0.2)); dictionary=str(dictionary or self.defaults.get('dictionary','4x4_1000'))
        if dictionary not in SUPPORTED_DICTIONARIES: raise ValueError(f'Unsupported dictionary: {dictionary}')
        ...
        t1=time.perf_counter(); markers=self._process_detections(image,detections,padding,max_hamming,dictionary,marker_size,camera_matrix,distortion); post_ms=(time.perf_counter()-t1)*1000.0
        return {'model':self.manifest['name'],'version':self.manifest['version'],'dictionary':dictionary,'image_size':{'width':int(image.shape[1]),'height':int(image.shape[0])},'settings':{'imgsz':imgsz,'conf':conf,'iou':iou,'max_hamming':max_hamming,'padding':padding,'dictionary':dictionary},'detections':len(detections),'markers':markers,'runtime_ms':{'detector':detector_ms,'postprocess':post_ms,'total':detector_ms+post_ms}}
    ...
    def _process_detections(self,image,detections,padding,max_hamming,dictionary,marker_size,camera_matrix,distortion):
